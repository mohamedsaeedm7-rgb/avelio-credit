...
export default function Pending() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        setError('');
        // Fetch only PENDING receipts
        const data = await apiGet('/receipts', { page, pageSize, status: 'PENDING' });
        ...
        setError(e.message || 'Failed to load pending receipts');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const totalPending = receipts.reduce((sum, r) => sum + parseFloat(r.amount || 0), 0);

  return (
    <div className="receipts-page">
      <div className="receipts-header">
        <div>
          <h2 className="receipts-title">Pending Receipts</h2>
          <p style={{ fontSize: '14px', color: '#64748B', margin: '8px 0 0 0' }}>
            {total} pending receipts • Total: ${totalPending.toFixed(2)}
          </p>
        </div>
      ...
      <div className="receipts-card">
        ...
        {loading ? (
          ...
          <div className="receipts-empty">No pending receipts found.</div>
        ) : (
          <>
              ...
              <table className="receipts-table">
                <thead>
                  <tr>
                    ...
                    <th className="receipts-th">Due Date</th>
                ...
                <tbody>
                  {receipts.map((receipt) => (
                    <tr key={receipt.id || receipt.receipt_number}>
                      ...
                      <td className="receipts-td">{receipt.due_date ? formatDT(receipt.due_date) : '-'}</td>
